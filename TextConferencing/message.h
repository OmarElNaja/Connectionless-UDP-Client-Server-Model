...
struct user {
    ...
    //int online;
    int sock_fd;
    char sessionID[MAX_NAME]; //-1 if not in session
};
...
int updateSession(struct user_list *user, struct sessions_list *s_list, struct sessions *new_session, struct user *username){
    while(user){
        if (user->user == username) {
            ...
            char old_session[MAX_DATA];
            if (new_session){
                strcpy(old_session, username->sessionID);
                strcpy(username->sessionID, new_session->sessionID); // update to new session
            } else {
                // session is null
                memset(username->sessionID, 0, MAX_NAME);
            }
            while (s_list){
                if(strcmp(s_list->session->sessionID, old_session) == 0) {
